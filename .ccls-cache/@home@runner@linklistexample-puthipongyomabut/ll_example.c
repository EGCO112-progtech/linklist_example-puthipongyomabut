...
{ 
   ...
   // delete first node
   if ( value == ( *sPtr )->data ) {  // sPtr is start
      tempPtr = *sPtr; // hold onto node being removed
      *sPtr = ( *sPtr )->nextPtr; // de-thread the node , *sPtr in position two
     if(*sPtr)
        (*sPtr)->pPtr=NULL;
...
// print the list
void reverseList( LLPtr currentPtr ){  // while two rounds 
   // if list is empty
   if ( isEmpty( currentPtr ) ) {
     ...
     while ( currentPtr->nextPtr!= NULL ) {
         currentPtr = currentPtr->nextPtr;} // หลังสุดเเล้ว

      // while not the end of the list
      while ( currentPtr->pPtr!= NULL ) { //ถอยจากหลังสุดมาหน้าสุด
